...
(define (grid-swap-cells grid p1 p2)
  (let* ((curr  (grid-ref grid (posn.x p1) (posn.y p1)))
         (next  (grid-ref grid (posn.x p2) (posn.y p2))))
    (grid-set! grid (posn.x p1) (posn.y p1) next)
    (grid-set! grid (posn.x p2) (posn.y p2) curr)))
